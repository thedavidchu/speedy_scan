...
public:
    ...
    explicit UniformRandom(uint64_t seed)
        : seed_(seed)
    {
    }
     ...
     */
    uint32_t
    uniform_within(uint32_t from, uint32_t to)
    {
        if (from == to) {
            return from;
        }
     ...
     */
    uint32_t
    uniform_within_except(uint32_t from, uint32_t to, uint32_t except)
    {
        while (true) {
     ...
     */
    uint32_t
    non_uniform_within(uint32_t A, uint32_t from, uint32_t to)
    {
        uint32_t C = get_c(A);
    ...
    uint64_t
    get_current_seed() const
    {
        return seed_;
    }
    void
    set_current_seed(uint64_t seed)
    {
        seed_ = seed;
    }
    uint64_t
