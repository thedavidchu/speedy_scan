// decoupled lookback implementation based on
...
static __device__ __forceinline__ Status_t
load_status(volatile const uint64_t *ptr)
{
    Status_t ret;
...
store_status(volatile uint64_t *ptr, Status_t val)
{
...
sum_chunk(int32_t thread_value, unsigned num_elems)
{
    ...
    // note: since we only have 512/32 = 16 partials, just do half a warp to
    // save some instructions.
    if (warp_idx == 0)
        partials[1 + lane_idx] =
            warp_prefix_sum<(CHUNK_SIZE / WARP_SIZE)>(partials[1 + lane_idx]);

    __syncthreads();

    // now each warp adds the prefix-summed subresult to its own subchunk of 32
    // values because of the +1 we did above, partials[0] contains 0, so it's a
    // no-op for the first warp.
    return result + partials[warp_idx];
}

static __device__ void
chunk_lookbehind_parallel(uint32_t block_idx,
                          ...
                          volatile int32_t *block_exclusive_sum)
{
    ...
    while (true) {
        // if the current lane does not have a predecessor, then it should not
        // affect the outcome -- ie. it should not be NONE.
        const auto pred_status = [&]() -> Status_t {
            if (block_idx > lane_idx + offset) {
                // note that this indexing scheme makes lane 0 control a higher
                // pred_idx than lane 31. for example, if we are at block 69,
                // then lane0 reads 68, lane1 reads 67, etc.
                return load_status(
                    &statuses[block_idx - offset - lane_idx - 1]);
            } else {
                return {0, ChunkState::Local};
            }
        }();

        // wait for all warps to do the thing. __all_sync should synchronise
        // (otherwise it's a shit name)
        const auto all_preds_aggregated =
            __all_sync(MASK_ALL, (pred_status.state != ChunkState::None));

        // if not all preds were aggregated, we need to keep waiting
        if (not all_preds_aggregated)
            continue;

        const auto finished_lanes =
            __ballot_sync(MASK_ALL,
                          (pred_status.state == ChunkState::Finished));
        const auto last_finished_pred_chunk = __ffs(finished_lanes);

        if (finished_lanes) {
            // first, sum all threads in the warp, then take the lane that
            // contains the correct value.
            const auto pred_sum = __reduce_add_sync(
                MASK_ALL,
                lane_idx < last_finished_pred_chunk ? pred_status.sum : 0);
...
__global__
__launch_bounds__(CHUNK_SIZE) void kernel(const int32_t *restrict in,
                                          ...
                                          uint32_t num_elems)
{
    ...
    // the compiler is apparently not smart enough to cache this (even though
    // it's not volatile), so do the caching manually so we stop hitting shared
    // memory
                            ...
                            size_t size)
{
    ...
    cuda_check(cudaMalloc((void **)&status_array,
                          num_chunks * sizeof(uint64_t) + sizeof(uint32_t)));
    cuda_check(cudaMemset(status_array,
                          0,
                          num_chunks * sizeof(uint64_t) + sizeof(uint32_t)));

    uint32_t *block_exec_counter = (uint32_t *)&status_array[num_chunks];

    kernel<<<num_chunks, CHUNK_SIZE>>>(d_input,
                                       d_output,
                                       status_array,
                                       block_exec_counter,
                                       size);
    ...
    cuda_check(cudaFree(status_array));
}
